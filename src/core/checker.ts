...
import { CHK_PATH, LANG_CONFIG, Verdict, RUN_USER_ID, RUN_GROUP_ID } from '../config'
...
class Checker extends Submission {
  ...
  async check(work_dir: string, exe_file: string = null, args: Array<string> = [], 
    ...
    function Timeout(ms: number): Promise<{ verdict: Verdict, code: number, signal: string }> {
    ...
    if (result.code === 0) return new Result(0, 0, result.code, 0, Verdict.Accepted);
