...
export default async function(sub_id: string, sub_code: string, sub_lang: string, 
  ...
  if (res.verdict === Verdict.CompileError || res.verdict === Verdict.SystemError) {
  ...
  for (let fingerprint of cases) {
    try {
      ...
      res.sum += 1;
      cache.set(sub_id, { verdict: Verdict.Judging, sum: res.sum }, 3600, () => {});
    } catch(ex) {
      sub.clear(); runner.clear();
      cache.set(sub_id, { verdict: Verdict.SystemError, message: ex.message }, 3600, () => {});
